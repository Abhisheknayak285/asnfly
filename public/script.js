// public/script.js - Client for Game Interface (with Reg Success Debug Logs)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function connectWebSocket() {
        ...
        window.socket.on('disconnect', (reason) => {
             // *** ADDED LOG with reason ***
            console.log(`[Client DEBUG] Disconnected from server. Reason: ${reason}`);
            showGameNotification('Disconnected from server!', 'error', 5000);
            if (gameStatusDisplay) gameStatusDisplay.textContent = "Disconnected";
            if (multiplierDisplay) multiplierDisplay.textContent = '---';
            if (mainActionButton) { mainActionButton.textContent = 'Offline'; mainActionButton.disabled = true; }
            setBetControlsDisabled(true); if (rocket) rocket.className = 'rocket-placeholder';
            if (cloudsBackground) cloudsBackground.classList.remove('clouds-active');
            currentBetThisRound = null; hasCashedOutThisRound = false; currentServerState = 'IDLE';
            window.socket = null;
         });
        ...
        window.socket.on('balanceUpdate', (data) => {
             // *** ADDED LOG ***
            console.log("[Client DEBUG] Received 'balanceUpdate'. Data:", JSON.stringify(data));
            if (balanceDisplay) { balanceDisplay.textContent = Math.floor(data.newBalance); } });
