// server.js - Final Version with fixed Session Table setup
...
const crypto = require('crypto'); // For generating recovery codes
    ...
    try {
        ...
        await client.query(`
            CREATE TABLE IF NOT EXISTS users (
                ...
                recovery_code_hash VARCHAR(60) NULL,
                created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP
            );
        `);
        console.log("Table 'users' checked/created.");
        // Ensure columns exist if table already did
        ...
        await client.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS recovery_code_hash VARCHAR(60) NULL;`);
        console.log("Columns for 'users' table checked/added.");
        ...
        // Define PRIMARY KEY directly in CREATE TABLE IF NOT EXISTS
        const createSessionTableQuery = `
        CREATE TABLE IF NOT EXISTS "session" (
            "sid" varchar NOT NULL COLLATE "default",
            "sess" json NOT NULL,
            "expire" timestamp(6) NOT NULL,
            CONSTRAINT "session_pkey" PRIMARY KEY ("sid") -- Define PK here
        );
        `;
        await client.query(createSessionTableQuery); // Create table (with PK if new)
        // Index can still be created conditionally afterwards
...
async function getUserData(username) { if (!username) return null; const query = 'SELECT id, username, password_hash, balance, status, email, email_verified, recovery_code_hash FROM users WHERE username = $1'; const values = [username.toLowerCase()]; let client; try { client = await pool.connect(); const result = await client.query(query, values); if (result.rows.length > 0) { const user = result.rows[0]; user.balance = parseFloat(user.balance); return user; } else { return null; } } catch (err) { console.error(`Error in getUserData for ${username}:`, err); return null; } finally { if (client) client.release(); } }
...
// Creates user and returns the new ID or null
async function createUserAndGetId(username, passwordHash, email) { if (!username || !passwordHash || !email) return null; const query = 'INSERT INTO users (username, password_hash, balance, status, email, email_verified) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id'; const values = [username.toLowerCase(), passwordHash, INITIAL_BALANCE, 'active', email.toLowerCase(), false]; let client; try { client = await pool.connect(); const result = await client.query(query, values); console.log(`[createUser] User ${username} created with ID: ${result.rows[0].id}`); return result.rows[0].id; } catch(err) { if (err.code === '23505') { console.log(`[createUser] Attempted to register existing username/email: ${username}/${email}`); } else { console.error(`Error creating user ${username}:`, err); } return null; } finally { if (client) client.release(); } }
// Saves recovery code hash for a user ID
async function saveRecoveryHash(userId, recoveryCodeHash) { if (!userId || !recoveryCodeHash) return false; const query = 'UPDATE users SET recovery_code_hash = $1 WHERE id = $2'; const values = [recoveryCodeHash, userId]; let client; try { client = await pool.connect(); await client.query(query, values); console.log(`[saveRecoveryHash] Saved recovery hash for user ID ${userId}`); return true; } catch (err) { console.error(`Error saving recovery hash for user ID ${userId}:`, err); return false; } finally { if (client) client.release(); } }
// Updates only the balance
...
function generateRecoveryCode(length = 10) { return crypto.randomBytes(Math.ceil(length / 2)).toString('hex').slice(0, length); }
...
app.post('/register', async (req, res) => { const { username, password, email } = req.body; if (!username || !password || !email || username.length < 3 || password.length < 6 || !email.includes('@')) { return res.status(400).json({ status: 'error', message: 'Valid username (>=3), password (>=6), and email required.' }); } console.log(`Registration attempt: ${username}`); const existingUser = await getUserData(username); if (existingUser) { return res.json({ status: 'error', message: 'Username already exists' }); } const existingEmailUser = await getUserByEmail(email); if (existingEmailUser) { return res.json({ status: 'error', message: 'Email address already registered.' }); } console.log(`Proceeding with registration for ${username}...`); let recoveryCode = ''; let userId = null; let client; try { client = await pool.connect(); await client.query('BEGIN'); const passwordHash = await bcrypt.hash(password, saltRounds); userId = await createUserAndGetId(username, passwordHash, email); if (!userId) throw new Error("User creation failed in transaction."); recoveryCode = generateRecoveryCode(10); const recoveryCodeHash = await bcrypt.hash(recoveryCode, saltRounds); const saveHashSuccess = await saveRecoveryHash(userId, recoveryCodeHash); if (!saveHashSuccess) throw new Error("Failed to save recovery code hash."); await client.query('COMMIT'); console.log(`Registration successful: ${username}. Recovery code generated.`); res.json({ status: 'success', message: 'Registration successful!', recoveryCode: recoveryCode }); } catch (err) { if (client) await client.query('ROLLBACK'); console.error("Error during registration processing:", err); if (err.message.includes("duplicate key value violates unique constraint")) { res.json({ status: 'error', message: 'Username or Email already exists.' }); } else { res.status(500).json({ status: 'error', message: 'Server error during registration' }); } } finally { if (client) client.release(); } });
...
io.on('connection', (socket) => {
    ...
    socket.on('forceDisconnect', (data) => { console.log(`Received forceDisconnect from server for socket ${socket.id}. Reason: ${data?.message}`); alert(`You have been disconnected: ${data?.message || 'Account status change.'}`); showAuthScreen(); }); // showAuthScreen needs to be globally accessible or handled differently if not
