// server.js - Admin Page for Deposits, Render PostgreSQL Backend
...
if (!process.env.SESSION_SECRET) {
    ...
    console.error("Please set a long, random string in Replit Secrets / Render Env Vars.");
    process.exit(1);
}
...
app.use(session({
    ...
    cookie: {
        ...
        secure: 'auto', // Use secure cookies automatically on HTTPS (like Render)
        httpOnly: true
    }
}));
...
let gameState = 'IDLE';
let currentMultiplier = 1.00; let crashPoint = 0; let gameStartTime = 0;
...
async function setupDatabase() {
    ...
    const createUserTableQuery = `CREATE TABLE IF NOT EXISTS users (...);`; // Keep from response #111
    const createDepositTableQuery = `CREATE TABLE IF NOT EXISTS deposit_requests (...);`; // Keep from response #131
    const createWithdrawalTableQuery = `CREATE TABLE IF NOT EXISTS withdrawal_requests (...);`; // Keep from response #131
    // SQL from connect-pg-simple documentation to create session table
    ...
    `;
    try {
        // Make sure CREATE TABLE queries match exactly the definitions from previous responses
...
// --- Authentication HTTP Routes (Modified Login to use Session) ---
app.post('/login', async (req, res) => {
    ...
    try {
        ...
        if (match) {
            ...
            req.session.user = {
                username: userData.username, // Store username from DB
                isAdmin: (userData.username.toLowerCase() === ADMIN_USERNAME.toLowerCase()) // Set admin flag
            };
            req.session.save(err => { // Save session before responding
                if (err) {
                    console.error("Session save error on login:", err);
                    return res.status(500).json({ status: 'error', message: 'Login failed (session error).' });
                }
                res.json({ status: 'success', username: userData.username });
            });
        } else {
             console.log(`Login failed (password mismatch): ${username}`);
             res.json({ status: 'error', message: 'Invalid username or password' });
        }
    } catch (compareError) { /* ... error handling ... */ res.status(500).json({ status: 'error', message: 'Server error during login' });}
});

app.post('/register', async (req, res) => { /* ... Keep register route from #113 (no session needed here) ... */
    const { username, password } = req.body; if (!username || !password || username.length < 3 || password.length < 6) { return res.status(400).json({ status: 'error', message: 'Username >= 3 chars, Password >= 6 chars.' }); } console.log(`Registration attempt: ${username}`); const existingUser = await getUserData(username); if (existingUser) { return res.json({ status: 'error', message: 'Username already exists' }); } console.log(`Proceeding with registration for ${username}...`); try { const passwordHash = await bcrypt.hash(password, saltRounds); const success = await createUser(username, passwordHash); if (success) { console.log(`Registration successful: ${username}`); res.json({ status: 'success', message: 'Registration successful! Please log in.' }); } else { console.error(`Registration failed for ${username}, potentially DB error.`); res.status(500).json({ status: 'error', message: 'Registration failed (server error).' }); } } catch (err) { console.error("Error during registration hashing/saving:", err); res.status(500).json({ status: 'error', message: 'Server error during registration' }); }
});

app.post('/logout', (req, res) => {
    req.session.destroy(err => { /* ... Keep logout route from #157 ... */
        if (err) { console.error("Error destroying session:", err); return res.status(500).json({ status: 'error', message: 'Logout failed' }); }
        res.clearCookie('connect.sid'); console.log("User logged out, session destroyed."); res.json({ status: 'success', message: 'Logged out' });
     });
});

// --- Admin Section ---

// Middleware to check if user is logged in as Admin via session
function isAdmin(req, res, next) {
    if (req.session && req.session.user && req.session.user.isAdmin) {
        return next(); // User is admin, proceed
    } else {
        // Not logged in or not admin - redirect or forbid
        console.warn("Unauthorized attempt to access admin route by:", req.session?.user?.username || 'Not logged in');
        // res.redirect('/'); // Option 1: Redirect to login/main page
        res.status(403).send('Forbidden: Admin access required.'); // Option 2: Send error
    }
}

// Serve the admin page, protected by isAdmin middleware
app.get('/admin', isAdmin, (req, res) => {
    // Ensure the admin.html file exists in public folder
    res.sendFile(path.join(__dirname, 'public', 'admin.html'));
});

// API Endpoint for admin to get pending deposit requests
app.get('/api/admin/pending-deposits', isAdmin, async (req, res) => {
    console.log("Admin requesting pending deposits...");
    const query = `SELECT id, username, transaction_id, status, requested_at FROM deposit_requests WHERE status = 'pending' ORDER BY requested_at ASC;`;
    let client;
    try {
        client = await pool.connect();
        const result = await client.query(query);
        res.json(result.rows); // Send the list
    } catch (err) { console.error("Error fetching pending deposits for admin:", err); res.status(500).json({ message: "Error fetching requests" }); }
    finally { if (client) client.release(); }
});

// API Endpoint for admin to approve a deposit
app.post('/api/admin/approve-deposit', isAdmin, async (req, res) => {
    const { requestId, verifiedAmount } = req.body;
    const adminUsername = req.session.user.username;
    const requestIdNum = parseInt(requestId);
    if (isNaN(requestIdNum) || typeof verifiedAmount !== 'number' || verifiedAmount <= 0) { return res.status(400).json({ status: 'error', message: 'Invalid Request ID or Verified Amount format.' }); }
    console.log(`ADMIN API: ${adminUsername} approving Request ID [${requestIdNum}] for amount [${verifiedAmount}]`);
    let client;
    try {
        client = await pool.connect(); await client.query('BEGIN'); // Start transaction
        const getRequestQuery = `SELECT * FROM deposit_requests WHERE id = $1 AND status = 'pending' FOR UPDATE`;
        const requestResult = await client.query(getRequestQuery, [requestIdNum]);
        if (requestResult.rows.length === 0) { throw new Error(`Request ID ${requestIdNum} not found or not pending.`); }
        const requestData = requestResult.rows[0]; const targetUsername = requestData.username;
        const userData = await getUserData(targetUsername); // Use helper, acquires own client
        if (!userData) { throw new Error(`Target user ${targetUsername} not found.`); }
        const currentBalance = userData.balance; const newBalance = currentBalance + verifiedAmount;
        const updateSuccess = await updateUserBalance(targetUsername, newBalance); // Use helper
        if (!updateSuccess) { throw new Error(`Failed to update balance for ${targetUsername}.`); }
...
// --- Socket.IO Connection Handling (No major changes needed here from #113) ---
io.on('connection', (socket) => {
    ...
    socket.on('authenticate', async (data) => {
        ...
        // *** Check session matches authenticated user ***
        // For slightly better security, verify the session user matches the username sent
        if (!req.session || !req.session.user || req.session.user.username.toLowerCase() !== username.toLowerCase()){
             console.warn(`Session user ${req.session?.user?.username} doesn't match authenticated user ${username} for socket ${socket.id}`);
             // Decide how to handle - disconnect or just don't authenticate? Let's disconnect.
...
async function startServer() {
    ...
    try {
        const client = await pool.connect();
        console.log("PostgreSQL DB connected for setup check.");
        await setupDatabase(); // Ensure tables exist (users, deposit_reqs, withdrawal_reqs, session)
        client.release();
        server.listen(port, () => { // Start web server AFTER setup check
         ...
         console.error(dbError); process.exit(1); // Exit if DB unavailable on start
    }
}
